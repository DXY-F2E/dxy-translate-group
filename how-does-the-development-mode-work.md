...
<details>
  <summary>再次阐明: 你可以在 browser 中使用 &lt;script&gt; 标签加载, React 和 Vue 都提供, 不依赖此(译者注: 指的是上述的 process 对象). 但是你需要手动的在 .js 和 生产的 .min.js 文件中选择. 接下来的只是关于通过一个打包器使用 React 和 Vue, 从 npm import 它们.</summary>
